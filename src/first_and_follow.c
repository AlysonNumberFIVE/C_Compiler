...
t_hashtable *first_and_follow(void)
{
	...
	ht_insert(ht, "(", "( sizeof char const void struct int short double float size_t long longlong signed void ID NUM LITERAL");
	ht_insert(ht, ";", " ( DONE } NUM sizeof return char const void struct int short double float size_t long longlong signed void if for while do");
	ht_insert(ht, "LITERAL", ") ; , } +");
	ht_insert(ht, "]", "; + - / = == *");
	ht_insert(ht, "[", "NUM sizeof");
	ht_insert(ht, "@", "ID");
	ht_insert(ht, "CHAR", ") ; , }");
