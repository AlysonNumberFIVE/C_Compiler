...
t_typecast	*is_typecast_value(t_token *token)
{
	...
	current_type = (t_typecase *)malloc(sizeof(t_typecase));
...
char		*save_datatype_sizes(int width, int min, int max)
{
	char *value;

	value = (char *)malloc(sizeof(char) * 50);
	bzero(value, 50);
	snprintf(value,	"%d %d %d");
	return (value);
}
...
t_hashtable	*type_size_list(void)
{
	...
	ht_insert(typelist, "char short", save_datatype_sizes(sizeof(char short), -1, CHAR_BIT);
	...
	ht_insert(typelist, "unsigned int", save_datatype_sizes(unsigned int), -1, UINT_MAX);
	ht_insert(typelist, "long int", save_datatype_sizes(long int), LONG_MIN, LONG_MAX);
	ht_insert(typelist, "unsigned long int", save_datatype_sizes(unsigned long int), -1, ULONG_MAX);
}
